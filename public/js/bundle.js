...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoiYnVpZ2Fib3IiLCJhIjoiY2tqbjVoemU0MGFsYzJ2bzh6NjZjdTlhNiJ9.paqLKmgDTa5Jy17ibtADvg';
  ...
  locations.forEach(function (location) {
    ...
    new mapboxgl.Marker({
      element: el,
      anchor: 'bottom'
    }).setLngLat(location.coordinates).addTo(map); // Add popup with information

    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(location.coordinates).setHTML("<p>Day ".concat(location.day, ": ").concat(location.description, "</p>")).addTo(map); // Extend map bounds to include current locations
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            stripe = Stripe('pk_test_51IDBdiK5GenrVqij8MiyRhpWUyZFHBnOMBkDjOIQ3byLGLmar9es5TnxjravGB74CGN9edULkLytKt2WpI0FAGu4002kBrtQma'); // 1) Get checkout session from API
